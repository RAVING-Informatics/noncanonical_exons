...
- See script `merge_pext_scores.sh` which uses `bigWigToBedGraph` to convert `.bw` files to `.bedgraph`, then uses `bedtools unionbedg` to find the union of the `.bedgraph` files.
